...
export class CartService {
    ...
    public calculateStockCounts(
        ...
        if (stock < qty) {
            ...
            this.snackBar.open(
                "No puedes agregar, sin stock. En stock " +
                    stock +
                    " artículos.",
                "×",
        ...
        return this.cartItems.pipe(
            map((product: CartItem[]) => {
                return products.reduce((prev, curr: CartItem) => {
                    const disc = curr.product.discount_uno
                        ? curr.product.discount_uno
                        : 0;
                    const price_calc = +curr.product.price - +curr.product.price * +disc / 100;
                    let total =  prev + (price_calc * curr.quantity);
                    let round =  Math.round(total)
                    // let totalRemoveComa = this.formatPipe.transform(round)
                    return round ;
                }, 0);
            })
        );
    ...
    public updateCartQuantity(
        ...
        return products.find((items, index) => {
            if (items.product.id == product.id) {
                let qty = products[index].quantity + quantity;
                let stock = this.calculateStockCounts(
                    products[index],
                    quantity
                );
